...
export class CopilotTracker implements vscode.Disposable {
    ...
    private ensureGitHook(repoPath: string, gitDir: string): void {
        try {
            // Check if there's a custom hooksPath configured
            let hooksDir: string;
            try {
                ...
                if (customHooksPath) {
                    // Use the custom hooks path (resolve relative paths)
                    hooksDir = path.isAbsolute(customHooksPath) 
                        ? customHooksPath 
                        : path.join(repoPath, customHooksPath);
                    this.logger.appendLine(`[Tracker] Using custom hooks path: ${hooksDir}`);
                } else {
...
`.replace(/\r\n/g, '\n');
            ...
            if (!fs.existsSync(hookFile)) {
                ...
                if (!currentContent.includes('AI_IMPACT_PENDING')) {
                    // Remove trailing 'exit 0' or 'exit' so our code runs
                    let modifiedContent = currentContent;
                    const exitMatch = modifiedContent.match(/\n(exit\s+0?\s*)$/);
                    if (exitMatch) {
                        modifiedContent = modifiedContent.slice(0, -exitMatch[0].length);
                    }
                    
                    const prefix = modifiedContent.endsWith('\n') ? '' : '\n';
                    const suffix = '\nexit 0\n';
                    fs.writeFileSync(hookFile, modifiedContent + prefix + hookContent + suffix);
                    this.logger.appendLine(`[Tracker] Updated git hook in ${hooksDir}`);
                }
            }
        } catch (e) {
