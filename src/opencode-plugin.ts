...
function checkConsumed(sess: SessionState): boolean {
    ...
    if (fs.existsSync(getConsumedPath(sess.gitDir))) {
        sess.consumed = true;
        // Keep consumed marker on disk — only clearConsumed removes it.
        // Clean stale state/flag so they don't bleed into manual commits.
...
export { AIContributionTracker, extractSessionId, getConsumedPath };
